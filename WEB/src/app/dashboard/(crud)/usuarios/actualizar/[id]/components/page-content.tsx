// type Props = {
//   localidadWithComuna: LocalidadWithComuna;
//   comunasData: Comuna[];
// };
...
import ButtonUpdateUsuario from "./button-update";
...
type Props = {
  perfilesData: Perfil[];
  usuariowithEstado: UsuarioWithProfileFlattened;
};
...
export default function PageContent({
  ...
  console.log(
    "🚀 ~ file: page-content.tsx:58 ~ PageContent ~ usuariowithEstado:",
    usuariowithEstado,
  );
  console.log(
  ...
  const [usuarioId, setUsuarioId] = useState<string>(
    usuariowithEstado?.USU_ID ?? "",
  );
  const [usuarioNombre, setUsuarioNombre] = useState<string>(
    usuariowithEstado?.USU_NOM ?? "",
  );
  const [usuarioUsername, setUsuarioUsername] = useState<string>(
    usuariowithEstado?.USU_USR ?? "",
  );
  const [usuarioApellidoPaterno, setUsuarioApellidoPaterno] = useState<string>(
    usuariowithEstado?.USU_APA ?? "",
  );
  const [usuarioApellidoMaterno, setUsuarioApellidoMaterno] = useState<string>(
    usuariowithEstado?.USU_AMA ?? "",
  );
  const [usuarioPhone, setUsuarioPhone] = useState<number>(
    usuariowithEstado?.USU_TEL ?? 0,
  );
  ...
  const [selectedPerfil, setSelectedPerfil] = useState<PerfilSelect>({
    id: usuariowithEstado?.PRF_ID ?? "",
    label: usuariowithEstado?.PRF_DSC ?? "",
    value: usuariowithEstado?.PRF_DSC ?? "",
  });
  ...
  useEffect(() => {
    ...
    setUsuarioId(usuariowithEstado.USU_ID);
  }, [usuariowithEstado]);
  ...
  useEffect(() => {
    if (
      (selectedPerfil?.id && typeof selectedPerfil?.id === "string") ||
      selectedPerfil?.id === ""
    ) {
  ...
  const handleNombreBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (usuarioNombreIsValid || e.target.value === "") {
  ...
  const handleApePaternoBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    const usuarioApePaternoIsValid = isOnlyString(e.target.value);
    if (usuarioApePaternoIsValid || e.target.value === "") {
      const capitaliedNombreLocalidad = capitalizeFirstLetter(e.target.value);
  ...
  const handleApeMaternoBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    const usuarioApeMaternoIsValid = isOnlyString(e.target.value);
    if (usuarioApeMaternoIsValid || e.target.value === "") {
      const capitaliedNombreLocalidad = capitalizeFirstLetter(e.target.value);
  ...
  const handleUsernameBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (!isUsernameValid) {
      ...
      setIsUsuarioUsernameValid({ status: false, message: "Usuario inválido" });
    }
  };
  ...
  const handlePhoneBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (usuarioPhoneIsValid || parseInt(e.target.value) === 0) {
          ...
          <div className="flex w-full flex-col gap-4 md:flex-row ">
            <div className="flex w-full flex-col items-start gap-4">
              <div className="flex w-full max-w-[350px] flex-col items-start justify-center gap-4">
                <Label htmlFor="rut" className="font-bold">
                  RUT
                </Label>
                <Input
                  type="text"
                  placeholder="ejemplo: Coquimbo"
                  id="rut"
                  name="rut"
                  required
                  className={cn(
