...
export default function SelectComuna({
  PerfilSelectData,
  selectedPerfil,
  setSelectedPerfil,
}: Props) {
  console.log(
    "🚀 ~ file: select-region.tsx:39 ~ selectedPerfil:",
    selectedPerfil,
  );
  console.log(
    "🚀 ~ file: select-region.tsx:39 ~ PerfilSelectData:",
    PerfilSelectData,
  );
  ...
  useEffect(() => {
    ...
    console.log("🚀 ~ file: select-region.tsx:45 ~ perfiles:", perfiles);

    if (selectedPerfil.value === "") {
      setValue("");
    } else {
      setValue(selectedPerfil.value);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [PerfilSelectData]);

  useEffect(() => {
    if (value === "") setSelectedPerfil({ id: "", label: "", value: "" });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [value]);
